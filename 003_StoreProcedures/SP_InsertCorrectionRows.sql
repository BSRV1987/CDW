Create or alter procedure sp_insertCorrectionrows
As
BEGIN
...
Select cte.contractid,pd.Product_Id,r.Region_Id,st.Status_id,cr.Cancellationreason_Id,cte.usage,cte.usagenet,(cte.[baseprice]+(cast(cte.usagenet as INT)*cte.[workingprice])),cte.createdat,cte.startdate,cte.enddate,cte.fillingdatecancellation,cte.AccountingPeriod,'Correction',getdate(),getdate() from Cte_dataprep cte
