Create or alter procedure sp_insertcontrarows @CurrentPeriod BIGINT
As
BEGIN
...
If not exists (select 1 from Fact.Contracts where AccountingPeriod=@maxperiod and [RecordType]='Contra') AND @maxperiod<>@CurrentPeriod
Begin
...
where AccountingPeriod=@maxperiod
End
